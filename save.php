...
if(isset($_SESSION["username"])) {
    ...
    if(isset($_POST["article_id"])){
        ...
        if($res && mysqli_num_rows($res)>0){
            $query="DELETE FROM read_later WHERE user_id='".$id."' AND article_id='".$article."'";
            $res=mysqli_query($conn, $query);

            if($res){
                echo"rimosso dalla lista";
                mysqli_close($conn);
                exit();
            }
        }else{
            
            $query = "INSERT INTO read_later (article_id, user_id) VALUES ('".$article."','".$id."')";
            $res = mysqli_query($conn, $query);
